...
-(void)showNoDataImage
{
    ...
    [self.view.subviews enumerateObjectsUsingBlock:^(UITableView* obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[UITableView class]]) {
            [self->_noDataView setFrame:CGRectMake(0, 0,obj.frame.size.width, obj.frame.size.height)];
            [obj addSubview:self->_noDataView];
        }
    }];
}
...
- (void) setIsShowLiftBack:(BOOL)isShowLiftBack
{
    ...
    if (isShowLiftBack && ( VCCount > 1 || self.navigationController.presentingViewController != nil)) {
        [self addNavigationItemWithImageNames:@[@"返回2"] isLeft:YES target:self action:@selector(backBtnClicked) tags:nil];
