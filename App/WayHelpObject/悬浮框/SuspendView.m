...
- (instancetype)initWithFrame:(CGRect)frame mainImageName:(NSString*)name bgcolor:(UIColor *)bgcolor{
    return [self initWithFrame:frame mainImageName:name bgcolor:bgcolor animationColor:nil AndSubview:nil];
}
- (instancetype)initWithFrame:(CGRect)frame mainImageName:(NSString*)name bgcolor:(UIColor *)bgcolor animationColor:animationColor AndSubview:(UIView *)VView
{
    if(self = [super initWithFrame:frame])
    {
        ...
        _mainImageButton.backgroundColor=[UIColor clearColor];
...
- (void)justbegin{
    ...
    CGPoint panPoint = CGPointMake(0, kScreenHeight-150-74);
...
- (void)locationChange:(UIPanGestureRecognizer*)p{
    CGPoint panPoint = [p locationInView:_MySubview];
...
- (void)changeStatus
{
    [UIView animateWithDuration:1.0 animations:^{
        self.mainImageButton.alpha = sleepAlpha;
    }];
...
- (void)buttonAnimation{
    ...
    _circleShape = [self createCircleShapeWithPosition:CGPointMake(width/2, height/2) pathRect:CGRectMake(0, 0, radius , radius ) radius:radius];
...
- (CAShapeLayer *)createCircleShapeWithPosition:(CGPoint)position pathRect:(CGRect)rect radius:(CGFloat)radius
{
    ...
    // 雷达覆盖区域
    circleShape.bounds = CGRectMake(0, 0, radius , radius );
